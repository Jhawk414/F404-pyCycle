...
class MPMixedFlowTurbofan(pyc.MPCycle):
    def setup(self):
        ...
        for i,pt in enumerate(self.od_pts):
            ...
            #self.set_input_defaults(pt+'.balance.rhs:FAR_ab', self.od_T7s[i], units='degR') #balances for augmentor temp
            self.set_input_defaults(pt+'.balance.rhs:W', self.od_Fns[i], units='lbf')
...
if __name__ == "__main__":
    ...
    prob = Problem() # Instantiates an OpenMDAO problem.
